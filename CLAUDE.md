# git-slicer — Agent Instructions

You have access to the `git-slicer` MCP server. Use it proactively to save context and tokens.

## Core rule

**Never read multiple files one-by-one when you can use a git-slicer tool instead.**

## When to use each tool

### `map_dependencies` — orient first
...
### `analyze_feature` — load context surgically
...
This replaces 5–20 individual file reads with a single tool call.
...
### `find_dependents` — assess impact before changing
...
```
1. map_dependencies  →  understand structure, find the right entry point
2. analyze_feature   →  load source of the relevant module
3. find_dependents   →  before any change, check impact
4. Read/Edit         →  now read or edit individual files as needed
```
